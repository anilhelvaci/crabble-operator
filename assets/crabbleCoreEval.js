...
const startCommittee = async (
    ...
    const voterInvitations = await E(committeeKit.creatorFacet).getVoterInvitations();

    logger('Sending committeeinvitations...');
    await Promise.all(
        zip(memberDeposits, voterInvitations).map(([depositFacet, invitation]) => E(depositFacet).receive(invitation)),
    );
...
const startCrabble = async (powers, config, crabbleNode, committeeInfoP) => {
    ...
    const {
        ...
        binaryVoteCounterInstallation: E(agoricNamesP).lookup('installation', 'binaryVoteCounter'),
    ...
    await Promise.all(
        memberDeposits.map(async df => {
            const inv = await E(kit.governorCreatorFacet).makeCommitteeMemberInvitation();
