...
const makeCrabbleFlowOffers = async (offerSender, instances, brands, issuers) => {
    const utilityAmount = AmountMath.make(brands['CrabbleCollection'], harden(scenarioConfigs.content[2]));

    const collateralAmount = AmountMath.make(brands['CrabbleIST'], harden(scenarioConfigs.content[1]));

    const rentalFeePerUnitAmount = AmountMath.make(brands['CrabbleIST'], harden(scenarioConfigs.content[1]));

    const rentalFeeAmount = AmountMath.make(
        brands['CrabbleIST'],
        harden(scenarioConfigs.rentals.buyouts[0].rentingDuration * scenarioConfigs.content[1]),
    );
